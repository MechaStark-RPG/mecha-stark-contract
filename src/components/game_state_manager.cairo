...
fn _validate_game(game_state: GameState, turns: Array<Turn>) -> GameResult {
    let mut mecha_dict = load_initial_state(game_state);
    let mut mecha_static_data = load_static_data(game_state);

    // validate game_state
    assert(game_state.mechas_state_player_1.len() == 5, 'game_s_manager: INVALID_LENGTH');
    assert(game_state.mechas_state_player_2.len() == 5, 'game_s_manager: INVALID_LENGTH');

    let player_1 = game_state.player_1;
    let mut ret = GameResult::Winner1(());
    let mut valid = true;
    let mut idx = 0;
    loop {
        if idx == turns.len() {
        ...
        loop {
            ...
            if !validate_and_execute_action(player, action, ref mecha_dict, ref mecha_static_data) {
                if player == game_state.player_1 {
                    ret = GameResult::Cheater1(());
                } else {
                    ret = GameResult::Cheater2(());
                }
                valid = false;
                break ();
            }
...
fn validate_and_execute_action(
    ...
    if mecha_dict.get_mecha_hp(action.mecha_id) == 0 {
        return false;
    }
    ...
    match action.first_action {
        TypeAction::Movement(()) => {
            if !action.movement.has_default_value() {
                ...
                mecha_dict.update_mecha_position(action.mecha_id, action.movement);
            }
            ...
            if !action.attack.has_default_value() {
                ...
                mecha_dict.update_mecha_hp(action.mecha_id, action.attack, ref mecha_static_data);
            }
        },
        TypeAction::Attack(()) => {
            if !action.attack.has_default_value() {
