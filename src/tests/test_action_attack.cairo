...
mod tests {
    ...
    fn happy_path() {
        ...
        assert(action.validate_attack(player_1, ref mecha_dict, ref mecha_data) == true, 'Attack is not valid');
    }
    ...
    fn attack_inside_the_map() {
        ...
        let action = Action {
            ...
            first_action: TypeAction::Attack(()), 
            movement: Position { x: 1, y: 2 }, 
            attack: Position { x: 30, y: 5 } 
        };

        assert(action.validate_attack(player_1, ref mecha_dict, ref mecha_data) == false, 'Attack is not valid');
    }
